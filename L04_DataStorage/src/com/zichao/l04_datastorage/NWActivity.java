...
public class NWActivity extends Activity {
	...
	public void testConnectionGet(View v) {
		...
		new Thread() {
			...
			public void run() {
				try {
					// 1) 得到Path, 并加上参数
					String path = et_network_url.getText().toString() + "?name=Tom&age=11";
					URL url = new URL(path);
					// 2) 打开连接,得到HttpURLConnection对象
					HttpURLConnection connection = (HttpURLConnection) url.openConnection();
					// 3) 设置请求方式; 连接超时; 读取数据超时
	...
	public void testConnectionPost(View v) {
		final ProgressDialog dialog = ProgressDialog.show(this, null, "请等待...");
	...
	/**
	 * 使用Volley提交get请求: 
	 * 1. 框架内部接收到请求后自动在后台使用分线程处理
	 * 2. 通过切换到主线程调用Listener的回调方法来返回结果数据
	 */
	public void testVolleyGet(View v) {
		final ProgressDialog dialog = ProgressDialog.show(this, null, "正在请求中...");
		String path = et_network_url.getText().toString() + "?name=Tom5&age=55";
		// 创建请求对象StringRequest:默认为GET
